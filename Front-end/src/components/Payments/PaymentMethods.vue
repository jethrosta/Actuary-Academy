...
async function setPaymentData(method, provider) {
    try {
        paymentData.value.method = method;
        paymentData.value.provider = provider;
    } catch (error) {
        console.log(error);
    }
}
...
async function storeLocalData(title, value) { localStorage.setItem(`${title}`, value) }

function toMakePayment(method, provider, routeName) {
    setPaymentData(method, provider)
        .then(storeLocalData('pendingPaymentData', JSON.stringify(paymentData.value))
            .then(() => {
...
function goToPayment(method, provider) {
    switch (method) {
        case 'bankTransfer':
            toMakePayment(method, provider, 'Transfer Bank');
            break;
        case 'directDebit':
            toMakePayment(method, provider, 'Kartu Debit');
            break;
        case 'creditCard':
            toMakePayment(method, provider, 'Kartu Kredit');
            break;
        case 'Digital Banking':
            storeLocalData('bank', provider).then(() => {
