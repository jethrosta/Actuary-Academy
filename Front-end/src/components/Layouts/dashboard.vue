...
<template>
  <header
    class="full-size bg-main_blue lg:block hidden top-0 sticky shadow-xl lg:text-lg md:text-md text-xs z-30 lg:px-4 px-1 pt-3">
    <div class="flex justify-center relative">
      ...
      <div class="absolute right-0 my-2 mr-10 text-white cursor-pointer">
        ...
        <svg class="inline-block w-4 h-4 align-middle" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
    ...
    <nav class="flex justify-between items-center text-white font-inter font-bold mx-auto lg:px-10 px-4">
      ...
      <div v-else id="UserButton" class="flex space-x-4 pb-3">
        ...
        <div>
          <RouterLink to="/register" class="bg-sec_blue hover:bg-[#15419f] rounded-md py-[0.625rem] px-[1.25rem] ">Daftar
          </RouterLink>
        </div>
      </div>
  ...
  <div
    class="small-size text-white items-start lg:hidden z-100 px-1 pb-8 pt-2 -mb-16 flex flex-row top-0 sticky md:text-md text-xs ">

    <div class="flex flex-1">
      <NavSideMenu class="px-4 pt-2" :items="productSubmenu" />
    </div>

    <RouterLink :to="{ name: 'Landingpage' }" class="main-logo flex flex-col flex-1 py-2">
    ...
    <div class="flex flex-row flex-1 text-white items-center justify-end cursor-pointer">
