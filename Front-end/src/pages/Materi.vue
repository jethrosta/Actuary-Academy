...
const showingVid = ref()

const activeMaterial = ref({})

const toggleVid = () => {
    showingVid.value = !showingVid.value
}

const toggleActive = (idx) => {
    videos.value.flatMap(video => video.active = false)
    videos.value[idx].active = !videos.value[idx].active
    activeMaterial.value = {
        category: videos.value[idx].category,
        title: videos.value[idx].title,
        url: videos.value[idx].url
    }
}
...
onMounted(async () => {
    const API_URL = import.meta.env.VITE_API_URL + 'courses/me';
    try {
        const response = await axios.get(API_URL, {
            withCredentials: true
        });

        const videoObjects = response.data.flatMap(course =>
            course.submateri.flatMap(submateri =>
                submateri.video.map(video => ({
                    category: submateri.title,
        ...
        videos.value = videos.value.concat(videoObjects);
        videos.value[0].active = true;

        activeMaterial.value = {
            category: videos.value[0].category,
            title: videos.value[0].title,
            url: videos.value[0].url
        }
    } catch (error) {
    ...
    try {
        pdf.value = await axios.get('https://aa-worker.actvideos.workers.dev/', {
            responseType: 'blob'
        });
        ...
        <div class="flex">
            <div class="w-full max-w-[26rem] mr-6">
                ...
                <div class="bg-main_blue text-white rounded-xl p-6 pt-7">
                    <div v-for="(video, index) in videos" @click="toggleActive(index)">
                        <div v-show="video.active">
                            <div class="bg-white rounded-xl flex items-center px-4 py-3 relative cursor-pointer">
                                <div
                                    class="absolute -top-5 left-1/2 z-1 -translate-x-1/2 flex items-center bg-sec_blue rounded-full p-3 font-bold w-8 h-8">
                                    {{ index + 1 }}
                                </div>
                                <div class="w-28 h-[4.5rem] bg-gray-300 rounded-xl mr-4 shrink-0">
                                    <img :src="`https://vz-8b85c55e-86f.b-cdn.net/${video.url}/thumbnail.jpg`">
                                </div>
                                <div class="row-auto">
                                    <div class="text-sec_blue text-lg font-bold">{{ video.category }}</div>
                                    <div class="text-gray-900">{{ video.title }}</div>
                                </div>
                            </div>
                        </div>
                        <div v-show="!video.active">
                            <div class="max-h-[32rem] mt-4 overflow-auto flex items-center mb-5 cursor-pointer">
                                <div
                                    class="bg-white text-sec_blue flex items-center justify-center text-xs font-bold w-6 h-6 rounded-full p-2">
                                    {{ index + 1 }}
                                </div>
            ...
            <div class="w-2/3 max-w-4xl">
                    ...
                    <div class="flex items-center">
                        <div class="w-4 h-4 mr-2">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 21 21" fill="none">
                                <path
                                    d="M10.8041 16.2733C16.9014 16.206 19.95 14.0898 19.95 8.92495C19.95 3.67495 16.8 1.57495 10.5 1.57495C4.20005 1.57495 1.05005 3.67495 1.05005 8.92495C1.05005 12.1559 2.24309 14.1938 4.62916 15.2834L3.15005 19.425L10.8041 16.2733Z"
                ...
                <div v-if="activeMaterial.url">
                    <div class="" v-show="!showingVid">
                        <!-- <video id="demo-player" class="cld-video-player" width="850" height="480" controls></video> -->
                        <div style="position:relative;padding-top:56.25%;"><iframe
                                :src="`https://iframe.mediadelivery.net/embed/160654/${activeMaterial.url}?autoplay=false&loop=false&muted=false&preload=true`"
                                loading="lazy" style="border:0;position:absolute;top:0;height:100%;width:100%;"
                                allow="accelerometer;gyroscope;autoplay;encrypted-media;picture-in-picture;"
                                allowfullscreen="true"></iframe></div>
                        <!-- <AdvancedVideo :cld-vid="cldVid" controls width="1080" :sources="vidSrc" /> -->
                    </div>
                    <div class="" v-show="showingVid">
                        <object data="https://aa-worker.actvideos.workers.dev/" type="application/pdf" width="100%"
                            height="500px">
                            <p>Unable to display PDF file. <a href="https://aa-worker.actvideos.workers.dev/">Download</a>
                                instead.</p>
                        </object>
                    </div>
                </div>
                    ...
                    <div class="text-sec_blue flex justify-between items-center text-lg font-bold border-b-2">
                        ...
                        <div class="flex items-center border-2 border-sec_blue rounded-full px-4 py-2">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="20" viewBox="0 0 16 20" fill="none">
                                <path
                                    d="M9 1H3C1.89543 1 1 1.89543 1 3V17C1 18.1046 1.89543 19 3 19H13C14.1046 19 15 18.1046 15 17V7M9 1L15 7M9 1V6C9 6.55228 9.44772 7 10 7H15M5 11H11M5 15H11"
                                    stroke="#0D1C9F" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                            </svg>
